    ...
    def __init__(self, results=None):
        ...
        def execute(
            ...
            **config
            ...
            """
            # Pre-evaluation
            # For GeotiffArchive datacubes, subsetting data is not feasible
            if type(datacube) == sq.datacube.GeotiffArchive:
                filter_check = False

            if filter_check:
                # Tbd: Add meta_timestamps as input to FilterProcessor, retrieved from datacube
                # For STACCube, meta_timestamps can be retrieved from src
                # For OpenDataCube, tbd
                # Retrieve required timestamps according to filter operations
                fp = FilterProcessor.parse(
                    self, datacube, mapping, space, time, **config
                )
                valid_timestamps = fp.optimize().execute
                if cache_data:
                    # FilterProcessor contains a FakeProcessor instance
                    cache = fp.fp.cache
                else:
                    cache = None
            else:
                if cache_data:
                    fp = FakeProcessor.parse(
                        self, datacube, mapping, space, time, **config
                    )
